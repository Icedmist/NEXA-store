    ...
    <DashboardLayout>
      <div className="max-w-md mx-auto">
        ...
        <div className="relative aspect-square max-w-xs mx-auto bg-foreground/5 rounded-2xl overflow-hidden border border-border mb-6 animate-fade-in stagger-1">
          ...
          {scanning && (
            <div
              className="absolute left-6 right-6 h-0.5 bg-accent transition-none"
              style={{ top: `${6 + (scanLine / 100) * 88}%` }}
            />
          )}
        </div>
          ...
          <div className="bg-card rounded-xl border border-border p-5 animate-scale-in">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                ...
                <p className="text-lg font-medium tabular-nums mt-1">KES {found.price.toLocaleString()}</p>
              </div>
            </div>
            <div className="flex gap-2">
              <button onClick={handleAdd} className="flex-1 h-10 bg-primary text-primary-foreground rounded-xl text-sm font-medium">
                Add to Cart
              </button>
              <button onClick={handleScan} className="flex-1 h-10 bg-muted text-foreground rounded-xl text-sm font-medium">
                Scan Again
              </button>
            </div>
          </div>
