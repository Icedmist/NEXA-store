...
export default function Stores() {
  ...
  const handleCopy = (code: string) => { navigator.clipboard.writeText(code); setCopied(code); setTimeout(() => setCopied(null), 1500); };

  const handleAdd = () => {
    if (!newStore.name || !newStore.location) return;
    const store: Store = { id: `s${Date.now()}`, name: newStore.name, location: newStore.location, code: `NX-${Math.floor(1000 + Math.random() * 9000)}`, status: 'active', revenue: 0, transactions: 0 };
            ...
            <div className="bg-card rounded-2xl border border-border p-6 w-full max-w-md animate-scale-in" onClick={e => e.stopPropagation()}>
              ...
              <div className="space-y-3">
                ...
                <input type="text" placeholder="Location" value={newStore.location} onChange={e => setNewStore(p => ({ ...p, location: e.target.value }))}
                  className="w-full h-10 px-4 rounded-xl border border-border bg-background text-sm font-light focus:outline-none focus:ring-2 focus:ring-ring/10" />
                <button onClick={handleAdd} className="w-full h-10 bg-primary text-primary-foreground rounded-xl text-sm font-medium">Create Store</button>
              </div>
            </div>
          </div>
        )}
