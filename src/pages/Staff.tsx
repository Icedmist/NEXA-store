      ...
      <div>
        ...
        <div className="space-y-2">
          {staffList.map((member, i) => (
            <div
              key={member.id}
              className={`bg-card rounded-xl border border-border p-4 flex items-center gap-4 animate-fade-in stagger-${Math.min(i + 1, 6)}`}
            >
              <div className={`
                w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 text-xs font-medium
